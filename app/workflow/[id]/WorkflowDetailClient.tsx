    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-dark-400 mb-6">Workflow not found</p>
      ...
      <nav className="border-b border-white/[0.06] bg-dark-950/80 backdrop-blur-xl sticky top-0 z-20">
        <div className="max-w-5xl mx-auto px-6 lg:px-8">
          <div className="flex justify-between items-center h-14">
            ...
            <Link href="/dashboard" className="text-sm text-dark-400 hover:text-white transition">← Dashboard</Link>
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-8 py-10">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-dark-400 mb-3 max-w-xl">{workflow.description}</p>
            <div className="flex items-center gap-4 text-xs text-dark-400">
              ...
              <span className="bg-white/[0.04] border border-white/[0.06] px-2 py-0.5 rounded-full">{workflow.category}</span>
            </div>
          </div>
              ...
              <div key={step.agentId + i} className="flex items-center gap-3 shrink-0">
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-dark-400 text-[10px]">{step.agent?.name}</div>
                  </div>
                </div>
        ...
        <div className="glass-card p-6 mb-6">
          ...
          <div className="space-y-3">
            {stepAgents.map((step, i) => (
              <div key={step.agentId + i} className="p-4 rounded-xl border border-white/[0.04] bg-white/[0.02]">
                <div className="flex items-center gap-2.5 mb-2">
                  ...
                  <div>
                    <span className="text-xs font-semibold text-white">Step {i + 1}: {step.label}</span>
                    <span className="text-[10px] text-dark-400 ml-2">{step.agent?.name}</span>
                  </div>
                </div>
                <p className="text-xs text-dark-400 leading-relaxed pl-[42px]">{step.inputs.task}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="glass-card p-6">
          ...
          <div className="space-y-2">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-xl border border-white/[0.04] bg-white/[0.02]">
                <div className="flex items-center gap-3">
                  <span className="h-2 w-2 rounded-full bg-emerald-400" />
                  <span className="text-xs text-dark-300">Run #{workflow.runs - i}</span>
                  <span className="text-[10px] text-dark-500">• {stepAgents.length} steps completed</span>
                </div>
                <span className="text-[10px] text-dark-500">{i === 0 ? workflow.lastRun : `${i + 1} days ago`}</span>
              </div>
            ))}
          </div>
        </div>
