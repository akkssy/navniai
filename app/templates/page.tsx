    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          ...
          <h1 className="heading-serif text-3xl font-bold tracking-tight mb-3 text-ink-700">Workflow Templates</h1>
          ...
          {templates.map((template) => (
            <Link key={template.id} href={`/workflow/builder?template=${template.id}`}
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[10px] font-medium text-ink-400 bg-surface-100 border border-surface-300 px-2 py-0.5 rounded-md">{template.category}</span>
              </div>
              <h3 className="text-sm font-semibold mb-1.5 text-ink-700 group-hover:text-accent-500 transition">{template.name}</h3>
              <p className="text-xs text-ink-400 mb-3 leading-relaxed">{template.description}</p>
              <div className="flex flex-wrap gap-1 mb-3">
                {template.steps.map((step, i) => {
                  const agent = getAgentForStep(step)
                  return (
                    <span key={step.agentId + i} className="text-[10px] bg-surface-100 border border-surface-300 text-ink-500 px-2 py-0.5 rounded-md flex items-center gap-1">
              ...
              <div className="flex items-center justify-between pt-3 border-t border-surface-300">
                <span className="text-[10px] text-ink-300">{template.runs.toLocaleString()} runs • {template.steps.length} steps</span>
                <span className="text-accent-500 text-xs flex items-center gap-1 group-hover:gap-2 transition-all">
